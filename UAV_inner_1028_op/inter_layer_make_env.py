...
class Runner:
    ...
    def inter_layer_env(self, lock):
        ...
        while True:
            ...
            # ---------- 打印（频率可调） ----------
            # if self.total_steps % self._print_every == 0:
            #     print(
            #         f"[step {self.total_steps:>6}] "
            #         f"AvgRewardRaw({self._avg_window}): {avg_reward_raw:.6f} | "
            #         f"AvgRewardNorm({self._avg_window}): {avg_reward_norm:.6f} | "
            #         f"AvgProfit({self._avg_window}): {avg_profit:.6f}"
            #     )
