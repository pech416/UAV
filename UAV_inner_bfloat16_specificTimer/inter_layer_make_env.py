...
# 说明：
# - 保持 bfloat16 训练链路不变
# - 在 inter_layer_env() 中加入六个关键阶段的时间累加（按 episode 累积）
# - 每个 episode 结束时返回 timing 字典，供外层汇总统计（主控脚本会按 UAV_NUM 进行平均/总计）
    ...
    def inter_layer_env(self, lock):
        ...
        while True:
            ...
            if lock:
                ...
                inter_layer_actions = [np.random.uniform(-1, 1, size=(self.args.action_dim_n[i],)).astype(np.float32)
            ...
            if done_condition:
                ...
                timings = {
                    ...
                    "store": t_store,
                    "train": t_train,
                    "total": t_action + t_move + t_assign + t_reward + t_store + t_train
                }
                return (have_dealt_task_num, total_profits, inter_episode_total_time,
                        self.users_number_covered, outer_layer_reward_norm, outer_layer_reward_raw, timings)
            else:
