...
class Actor(nn.Module):
    def __init__(self, args, agent_id):
        ...
        # Ensure model parameters use bfloat16 precision
...
class Critic_MADDPG(nn.Module):
    def __init__(self, args):
        ...
        if args.use_orthogonal_init:
            ...
            orthogonal_init(self.fc2)
            orthogonal_init(self.fc3)
        # Ensure critic network parameters use bfloat16 precision
        self.to(torch.bfloat16)
    ...
    def forward_sa(self, s_a):
        # Optimization: directly compute Q from pre-concatenated state-action
        q = F.relu(self.fc1(s_a))
        q = F.relu(self.fc2(q))
        q = self.fc3(q)
        return q
...
class Critic_MATD3(nn.Module):
    ...
    def forward_sa(self, s_a):
        # Optimization: directly compute two Q values from pre-concatenated input
        q1 = F.relu(self.fc1(s_a))
        q1 = F.relu(self.fc2(q1))
        q1 = self.fc3(q1)
        q2 = F.relu(self.fc4(s_a))
