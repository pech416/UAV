...
class MADDPG(object):
    ...
    # --------------------- 动作选择（含高斯噪声） ---------------------
    def choose_action(self, obs, noise_std: float = 0.1, timing=None):
        """
        obs: np.ndarray shape=(obs_dim,)
        return: np.ndarray shape=(action_dim,), clipped to [-max_action, max_action]
        timing: optional dict to accumulate fine-grained timing (seconds). Expected keys:
            'actor_fc1', 'actor_fc2', 'actor_fc3', 'noise_add'
        """
        self.actor.eval()
        with torch.no_grad():
            obs_tensor = torch.as_tensor(obs, dtype=torch.float32).unsqueeze(0)  # (1, obs_dim)

            # 若启用计时，手动拆分 Actor 前向，统计 FC1/FC2/FC3 的耗时
            if timing is not None and bool(getattr(self.args, "enable_timing", False)):
                t0 = time.perf_counter()
                x = F.relu(self.actor.fc1(obs_tensor))
                t1 = time.perf_counter()
                timing["actor_fc1"] = timing.get("actor_fc1", 0.0) + (t1 - t0)

                t0 = time.perf_counter()
                x = F.relu(self.actor.fc2(x))
                t1 = time.perf_counter()
                timing["actor_fc2"] = timing.get("actor_fc2", 0.0) + (t1 - t0)

                t0 = time.perf_counter()
                a_tensor = self.max_action * torch.tanh(self.actor.fc3(x))
                t1 = time.perf_counter()
                timing["actor_fc3"] = timing.get("actor_fc3", 0.0) + (t1 - t0)
            else:
        ...
        if noise_std is not None and noise_std > 0:
            if timing is not None and bool(getattr(self.args, "enable_timing", False)):
                ...
                a = a + noise
                t1 = time.perf_counter()
                timing["noise_add"] = timing.get("noise_add", 0.0) + (t1 - t0)
            else:
                noise = np.random.normal(0.0, noise_std, size=self.action_dim).astype(np.float32)
                a = a + noise
