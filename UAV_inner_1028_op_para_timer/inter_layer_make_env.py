...
# ------------------------------ Episode Timing ----------------------------------
class EpisodeTiming:
    """Accumulate per-episode timing (seconds) and provide ms formatting."""
    def __init__(self):
        self.reset()
    ...
    def ms(self, key: str) -> float:
        return float(self.t.get(key, 0.0) * 1000.0)

    def print_console(self, episode_idx: int, total_ms: float, print_substages: bool = True):
        # 输出格式尽量贴近用户提供的示例图
        print("=" * 44)
        print("===== Episode Timing (ms) =====")
        print(f"1. State Norm:        {self.ms('state_norm'):.4f}")
        print(f"2. Action Select:     {self.ms('action_select'):.4f}")
        if print_substages:
            # 子阶段时间之和可能小于 Action Select（Python 循环等开销不计入子项）
            print(f"   - 2.1 Actor FC1 (NN):      {self.ms('actor_fc1'):.4f}")
            print(f"   - 2.2 Actor FC2 (NN):      {self.ms('actor_fc2'):.4f}")
            print(f"   - 2.3 Actor FC3 (NN):      {self.ms('actor_fc3'):.4f}")
            print(f"   - 2.4 Noise Addition:      {self.ms('noise_add'):.4f}")
        print(f"3. UAV Move:          {self.ms('uav_move'):.4f}")
        print(f"4. Task Assign:       {self.ms('task_assign'):.4f}")
        if print_substages:
            print(f"   - 4.1 Distance (sqrt):     {self.ms('dist_sqrt'):.4f}")
            print(f"   - 4.2 Trans Rate (log/pow):{self.ms('trans_rate'):.4f}")
            print(f"   - 4.3 Cache Check (loop):  {self.ms('cache_check'):.4f}")
            print(f"   - 4.4 Comp Time (scalar):  {self.ms('comp_time'):.4f}")
        print(f"5. Reward Calc:       {self.ms('reward_calc'):.4f}")
        print(f"6. Next State:        {self.ms('next_state'):.4f}")
        print(f"7. Store & Train:     {self.ms('store_train'):.4f}")
        print(f"Total Episode Time:   {total_ms:.4f}")
        print("=" * 44)


def find_best_uav_for_tasks_timed(uavs, users, cloud_position, timing: EpisodeTiming):
    """Timed version of task assignment to provide sub-stage breakdown."""
    uav_pos = np.array([u.position for u in uavs], dtype=np.float32)
    user_pos = np.array([u.position for u in users], dtype=np.float32)
    ...
    d_uav_user, d_uav_cloud = VO.calculate_distances_batch(uav_pos, user_pos, cloud_position)
    timing.add('dist_sqrt', time.perf_counter() - _t0)

    # 4.2 Trans Rate (log/pow)
    u0 = uavs[0]
    params = dict(P_u=u0.P_u, P_v_w=u0.P_v_w, r=u0.r, sigma2=u0.sigma2, N=u0.N, B=u0.B, B_cloud=u0.B_cloud)
    _t0 = time.perf_counter()
    r_user, r_cloud = VO.calculate_transmission_rates_batch(d_uav_user, d_uav_cloud, params)
    timing.add('trans_rate', time.perf_counter() - _t0)
    ...
    for ui, user in enumerate(users):
        for ti, task in enumerate(user.tasks):
            ...
            times = np.empty(n_uavs, dtype=np.float32)
            for k, u in enumerate(uavs):
                # 4.3 Cache Check (loop)
                _tc0 = time.perf_counter()
                in_content = (task[0] in u.cached_content_type)
                in_service = (not in_content) and (task[0] in u.cached_service_type)
                timing.add('cache_check', time.perf_counter() - _tc0)

                # 4.4 Comp Time (scalar)
                _ts0 = time.perf_counter()
                if in_content:
                    base = 0.0
                elif in_service:
                    computing_capacity = np.random.randint(3, 8) * u.F_cpu / 30.0
                    base = task[1] / computing_capacity
                else:
                    base = 2.0 * (task[1] / max(float(r_cloud[k]), 1e-6))

                total = base + (task[1] / max(float(r_user[k, ui]), 1e-6)) + (task[1] * 0.6) / u.U_V_transmission_rate
                timing.add('comp_time', time.perf_counter() - _ts0)
...
class Runner:
    ...
    def inter_layer_env(self, lock):
        ...
        while True:
            ...
            # -------- 选择动作 --------
            _t0 = time.perf_counter()
            action_sub = {"actor_fc1": 0.0, "actor_fc2": 0.0, "actor_fc3": 0.0, "noise_add": 0.0}
            if lock:
                inter_layer_actions = [ag.choose_action(obs, noise_std=self.noise_std, timing=action_sub)
            ...
            # 累加子阶段（FC 层/噪声）
            timing.add('actor_fc1', action_sub.get('actor_fc1', 0.0))
            timing.add('actor_fc2', action_sub.get('actor_fc2', 0.0))
            timing.add('actor_fc3', action_sub.get('actor_fc3', 0.0))
            timing.add('noise_add', action_sub.get('noise_add', 0.0))
            ...
            _t0 = time.perf_counter()
            assignments = find_best_uav_for_tasks_timed(self.uavs, self.users, cloud_position, timing)
        ...
        # -------- Episode 计时输出 --------
        episode_ms = (time.perf_counter() - episode_t0) * 1000.0

        # 打印（可控）
        if bool(getattr(self.args, "enable_timing", False)):
            every = int(getattr(self.args, "timing_print_every", 1))
            if every <= 1 or (episode_idx % every == 0):
                timing.print_console(
                    episode_idx=episode_idx,
                    total_ms=episode_ms,
                    print_substages=bool(getattr(self.args, "timing_print_substages", True))
                )

        # TensorBoard 记录 episode 总耗时
        if self.writer:
            self.writer.add_scalar("episode/time_ms", episode_ms, self.total_steps)

        # 返回：在原 6 项基础上，额外附加 episode_ms（单位 ms）
