...
class Actor(nn.Module):
    def __init__(self, args, agent_id):
        ...
        self.to(device=device, dtype=torch.float32)  # 修改：Actor 参数使用 bfloat16 精度
...
class Critic_MADDPG(nn.Module):
    def __init__(self, args):
        ...
        self.to(device=device, dtype=torch.float32)  # 修改：Critic_MADDPG 参数使用 bfloat16
...
class Critic_MATD3(nn.Module):
    def __init__(self, args):
        ...
        # 转换参数精度为 bfloat16
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 修改：选择设备
        self.to(device=device, dtype=torch.float32)  # 修改：Critic_MATD3 参数使用 bfloat16
