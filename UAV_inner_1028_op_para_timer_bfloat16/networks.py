...
class Actor(nn.Module):
    def __init__(self, args, agent_id):
        ...
        # 将模型参数转换为 bfloat16 并移动到加速器设备上
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 修改：选择设备（GPU 优先）
        self.to(device=device, dtype=torch.bfloat16)  # 修改：Actor 参数使用 bfloat16 精度
...
class Critic_MADDPG(nn.Module):
    def __init__(self, args):
        ...
        if args.use_orthogonal_init:
            ...
            orthogonal_init(self.fc3)
        # 转换参数精度为 bfloat16
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 修改：选择设备
        self.to(device=device, dtype=torch.bfloat16)  # 修改：Critic_MADDPG 参数使用 bfloat16
    ...
    def forward_sa(self, s_a):
        # 优化: 使用预拼接的 state-action 张量直接计算 Q 值，避免重复拼接
        q = F.relu(self.fc1(s_a))
        q = F.relu(self.fc2(q))
        q = self.fc3(q)
        return q
...
class Critic_MATD3(nn.Module):
    def __init__(self, args):
        ...
        # 转换参数精度为 bfloat16
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # 修改：选择设备
        self.to(device=device, dtype=torch.bfloat16)  # 修改：Critic_MATD3 参数使用 bfloat16
    ...
    def forward_sa(self, s_a):
        # 优化: 使用预拼接的 state-action 输入直接计算两个 Q 值
        q1 = F.relu(self.fc1(s_a))
        q1 = F.relu(self.fc2(q1))
        q1 = self.fc3(q1)
        q2 = F.relu(self.fc4(s_a))
