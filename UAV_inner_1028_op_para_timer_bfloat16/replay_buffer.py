...
class ReplayBuffer(object):
    ...
    def sample(self):
        ...
        for agent_id in range(self.N):
            batch_obs_n.append(torch.from_numpy(self.buffer_obs_n[agent_id][index]).to(device=self.device, dtype=torch.bfloat16))         # 修改：obs 转换为 bfloat16 张量
            batch_a_n.append(torch.from_numpy(self.buffer_a_n[agent_id][index]).to(device=self.device, dtype=torch.bfloat16))             # 修改：action 转换为 bfloat16 张量
            batch_r_n.append(torch.from_numpy(self.buffer_r_n[agent_id][index]).to(device=self.device, dtype=torch.bfloat16))             # 修改：reward 转换为 bfloat16 张量
            batch_obs_next_n.append(torch.from_numpy(self.buffer_s_next_n[agent_id][index]).to(device=self.device, dtype=torch.bfloat16)) # 修改：next_obs 转换为 bfloat16 张量
            batch_done_n.append(torch.from_numpy(self.buffer_done_n[agent_id][index]).to(device=self.device, dtype=torch.bfloat16))       # 修改：done 转换为 bfloat16 张量
