    ...
    def inter_layer_env(self, lock):
        ...
        while True:
            ...
            for user in self.users:
                ...
                d_u_cloud_arr = np.sqrt((pos_array[:, 0] - cloud_position[0])**2 +
                                       (pos_array[:, 1] - cloud_position[1])**2)
                ...
                trans_cloud_arr = 10 * B_cloud_arr * np.log2(1 + 10000 * P_u_arr *
                                                           np.power(d_u_cloud_arr/100.0, -r_arr) / (sigma2_arr + N_arr))
                trans_user_arr = 100 * B_arr * np.log2(1 + 5000 * (P_v_w_arr *
                                                               np.power(d_u_v_arr/10.0, -r_arr) / (sigma2_arr + N_arr)))
            ...
            assignment_time += time.perf_counter() - t0
            ...
            agent_rewards_norm = [float(np.clip((r - self.reward_rms.mean) / (self.reward_rms.std + 1e-8), -5, 5))
            ...
            if lock:
                ...
                if self.replay_buffer.current_size >= self.args.batch_size:
                    ...
                    training_time += time.perf_counter() - t_train
            else:
        ...
        # ---------- Episode Timing Output ----------
        print("[Episode Timing]")
        print(f"Action selection: {action_time:.3f}s")
        print(f"UAV movement: {move_time:.3f}s")
        print(f"Task assignment: {assignment_time:.3f}s")
        print(f"Reward calc: {reward_time:.3f}s")
        print(f"ReplayBuffer store: {store_time:.3f}s")
        print(f"Training: {training_time:.3f}s")
        total_time = action_time + move_time + assignment_time + reward_time + store_time + training_time
        print(f"Total: {total_time:.3f}s")

        # TensorBoard logging for timing (optional)
        if self.writer:
            self.writer.add_scalar("timing/action_selection", action_time, self.total_steps)
            self.writer.add_scalar("timing/uav_movement", move_time, self.total_steps)
            self.writer.add_scalar("timing/task_assignment", assignment_time, self.total_steps)
            self.writer.add_scalar("timing/reward_calc", reward_time, self.total_steps)
            self.writer.add_scalar("timing/replay_store", store_time, self.total_steps)
            self.writer.add_scalar("timing/training", training_time, self.total_steps)
            self.writer.add_scalar("timing/total", total_time, self.total_steps)

        return have_dealt_task_num, total_profits, inter_episode_total_time, self.users_number_covered, outer_layer_reward_norm, outer_layer_reward_raw
